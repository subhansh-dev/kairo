"""Tests for kairo.agent.memory_sqlite — SQLite-backed memory stores."""
...
from kairo.agent.memory_types import ProceduralSkill
...
def test_episodic_record_and_recent(tmp_path: Path):
    mem = EpisodicMemorySQLite(tmp_path / "e.db")
    mem.record("tool_call", "called read_file", path="foo.py")
    mem.record("tool_result", "got content")
...
def test_episodic_persists_across_connections(tmp_path: Path):
    p = tmp_path / "e.db"
    mem = EpisodicMemorySQLite(p)
    mem.record("x", "hello")
    mem.close()
    mem2 = EpisodicMemorySQLite(p)
    assert len(mem2.recent()) == 1
    assert mem2.recent()[0].summary == "hello"
    mem2.close()


def test_episodic_count(tmp_path: Path):
    mem = EpisodicMemorySQLite(tmp_path / "e.db")
    for i in range(100):
...
def test_semantic_add_and_query(tmp_path: Path):
    mem = SemanticMemorySQLite(tmp_path / "s.db")
...
def test_semantic_persists(tmp_path: Path):
    p = tmp_path / "s.db"
    mem = SemanticMemorySQLite(p)
    mem.add("a", "b", "c")
    mem.close()
    mem2 = SemanticMemorySQLite(p)
    assert len(mem2.query(subject="a")) == 1
    mem2.close()


def test_semantic_count(tmp_path: Path):
    mem = SemanticMemorySQLite(tmp_path / "s.db")
    mem.add("a", "b", "c")
    mem.add("d", "e", "f")
    assert mem.count() == 2
...
def test_procedural_add_and_get(tmp_path: Path):
    mem = ProceduralMemorySQLite(tmp_path / "p.db")
    ...
    assert got.name == "fix_syntax_error"
    assert got.steps == ["read_file", "find the line", "edit_file with corrected syntax"]
    mem.close()
...
def test_procedural_persists(tmp_path: Path):
    p = tmp_path / "p.db"
    mem = ProceduralMemorySQLite(p)
    mem.add(ProceduralSkill(
        id="s1", name="x", description="d", trigger="t", steps=["a"]
    ))
    mem.close()
    mem2 = ProceduralMemorySQLite(p)
    assert mem2.get("s1") is not None
    mem2.close()


def test_procedural_count(tmp_path: Path):
    mem = ProceduralMemorySQLite(tmp_path / "p.db")
    mem.add(ProceduralSkill(id="s1", name="a", description="d", trigger="t", steps=[]))
    mem.add(ProceduralSkill(id="s2", name="b", description="d", trigger="t", steps=[]))
    assert mem.count() == 2
    mem.close()


# ---------------------------------------------------------------------------
# AgentMemorySQLite (combined)
# ---------------------------------------------------------------------------

def test_agent_memory_load_creates_db_files(tmp_path: Path):
    am = AgentMemorySQLite.load(tmp_path)
    assert (tmp_path / "memory").is_dir()
    assert (tmp_path / "memory" / "episodic.db").exists()
    am.close()
