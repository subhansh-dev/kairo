...
function validateCommandPaths(
  ...
  if (validator && !validator(args)) {
    return {
      behavior: 'ask',
      message: `${command} with flags requires manual approval to ensure path safety. For security, Kairo cannot automatically validate ${command} commands that use flags, as some flags like --target-directory=PATH can bypass path validation.`,
      decisionReason: {
        type: 'other',
  ...
  if (compoundCommandHasCd && operationType !== 'read') {
    return {
      behavior: 'ask',
      message: `Commands that change directories and perform write operations require explicit approval to ensure paths are evaluated correctly. For security, Kairo cannot automatically determine the final working directory when 'cd' is used in compound commands.`,
      decisionReason: {
        type: 'other',
        reason:
    ...
    if (!allowed) {
      ...
      const message =
        ...
        decisionReason?.type === 'safetyCheck'
          ...
          : `${command} in '${resolvedPath}' was blocked. For security, Kairo may only ${ACTION_VERBS[command]} the allowed working directories for this session: ${dirListStr}.`
...
function validateOutputRedirections(
  ...
  if (compoundCommandHasCd && redirections.length > 0) {
    return {
      behavior: 'ask',
      message: `Commands that change directories and write via output redirection require explicit approval to ensure paths are evaluated correctly. For security, Kairo cannot automatically determine the final working directory when 'cd' is used in compound commands.`,
      decisionReason: {
        type: 'other',
        reason:
      ...
      const message =
        ...
        decisionReason?.type === 'safetyCheck'
          ...
          : decisionReason?.type === 'rule'
            ...
            : `Output redirection to '${resolvedPath}' was blocked. For security, Kairo may only write to files in the allowed working directories for this session: ${dirListStr}.`
