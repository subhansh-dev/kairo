...
function logLoadOnce(success: boolean): void {
  if (logged) return
  logged = true
  // kairo: debug logging removed (no analytics)
}
...
export async function ensureInitialized(): Promise<void> {
  // kairo: tree-sitter not available on Node.js — falls back to regex/shell-quote path
}
...
export async function parseCommand(
  ...
  // kairo: tree-sitter not available — always falls back to legacy path
  return null
}
