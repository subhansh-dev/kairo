/**
 * Kairo — Kimi (Moonshot) Dialect
 * Handles Kimi's thinking mode and tool calling format
 */

import type { DialectDefinition, Message, StreamOptions, ToolCall, ToolResult, AssistantMessageEvent } from '../types.js';

export const kimiDialect: DialectDefinition = {
  dialect: 'kimi',
  ...
  renderToolResults(results: ToolResult[]): string {
    return results.map(r =>
      `<tool_result>${r.content}</tool_result>`
    ).join('\n');
  },
